...
int ff_qsv_get_surface_base_handle(mfxFrameSurface1 *surf,
                                   ...
                                   void **base_handle)
{
#if CONFIG_VAAPI || CONFIG_D3D11VA || CONFIG_DXVA2
...
static int qsv_d3d11_update_config(void *ctx, mfxHDL handle, mfxConfig cfg)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (FAILED(hr)) {
            av_log(ctx, AV_LOG_ERROR, "Error IDXGIDevice_GetAdapter %ld\n", hr);
        ...
        if (FAILED(hr)) {
            av_log(ctx, AV_LOG_ERROR, "Error IDXGIAdapter_GetDesc %ld\n", hr);
            goto fail;
        }
    } else {
        av_log(ctx, AV_LOG_ERROR, "Error ID3D11Device_QueryInterface %ld\n", hr);
        return ret;
    }
...
static int qsv_d3d9_update_config(void *ctx, mfxHDL handle, mfxConfig cfg)
{
    ...
    if (FAILED(hr)) {
        av_log(ctx, AV_LOG_ERROR, "Error OpenDeviceHandle %ld\n", hr);
        goto fail;
    }

    hr = IDirect3DDeviceManager9_LockDevice(devmgr, device_handle, (void *)&device, TRUE);
    if (FAILED(hr)) {
        av_log(ctx, AV_LOG_ERROR, "Error LockDevice %ld\n", hr);
    ...
    if (FAILED(hr)) {
        av_log(ctx, AV_LOG_ERROR, "Error IDirect3DDevice9_GetCreationParameters %ld\n", hr);
        IDirect3DDevice9Ex_Release(device);
        goto unlock;
    }

    hr = IDirect3DDevice9Ex_GetDirect3D(device, (void *)&d3d9ex);
    if (FAILED(hr)) {
        av_log(ctx, AV_LOG_ERROR, "Error IDirect3DDevice9Ex_GetAdapterLUID %ld\n", hr);
        IDirect3DDevice9Ex_Release(device);
        goto unlock;
    }
                                        ...
                                        int flags)
{
    ...
    ret = qsv_create_mfx_session(ctx, handle, handle_type, implementation, &ver,
                                 &hwctx->session, (void **)&hwctx->loader);
    if (ret)
        goto fail;
